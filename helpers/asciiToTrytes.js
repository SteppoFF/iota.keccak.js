...
function toTrytes(input) {
    ...
    for (var i = 0; i < input.length; i++) {
        var asciiValue = input[i].charCodeAt(0);
        ...
        trytes += TRYTE_VALUES[firstValue] + TRYTE_VALUES[((asciiValue - firstValue) / 27)];
    }
    return trytes;
}
...
function fromTrytes(inputTrytes) {
    ...
    var TRYTE_VALUES = "9ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    var outputString = "";
    for (var i = 0, max=inputTrytes.length; i < max; i += 2) {
        // get a trytes pair
        outputString += String.fromCharCode((TRYTE_VALUES.indexOf(inputTrytes[i]) + TRYTE_VALUES.indexOf(inputTrytes[i + 1]) * 27));
    }
